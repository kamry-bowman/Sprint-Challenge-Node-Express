...
const stringToBoolean = require('../utilities/stringToBoolean');
...
const makeRouter = (type, shape, db, passedMiddlewares = []) => {
  ...
  // Binds the getError import with the type and shape p  arameters from makeRouter
  ...
  router.post('/', async (req, res, next) => {
    ...
    try {
      // convert any "false", "true" occurences in payload to boolean counterparts
      payload = stringToBoolean(payload);
      ...
      res.status(201);
      req.locals = { payload: dbGetResponse };
      next();
    } catch (err) {
  ...
  router.put('/:id', async (req, res, next) => {
    ...
    try {
      ...
      res.status(201);
      req.locals = { payload: dbResponsePost };
      next();
    } catch (err) {
